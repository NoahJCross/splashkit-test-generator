...
fn test_pop_clip_integration() {
    ...
    refresh_screen();
    assert_eq!(color_red(), get_pixel_from_window(test_window, 125.0, 125.0));
    assert_eq!(color_green(), get_pixel_from_window(test_window, 350.0, 350.0));
    close_window(test_window);
}
#[test]
...
fn test_option_with_animation_integration() {
    ...
    let test_bitmap = create_bitmap("test_bitmap".to_string(), 100, 100);
    clear_bitmap(test_bitmap, color_black());
    let test_animation = create_animation_from_script_named("test_animation".to_string(), "default".to_string());
...
fn test_display_details_integration() {
    ...
    assert!(displays > 0);
    let display = display_details(0u32);
...
fn test_refresh_screen_with_target_fps_integration() {
    ...
    clear_window(test_window, color_white());
    draw_circle_record(color_black(), circle_at_from_points(400.0, 300.0, 50.0));
    refresh_screen_with_target_fps(60u32);
    close_window(test_window);
}
#[test]
...
fn test_bitmap_height_of_bitmap_named_integration() {
    let test_bitmap = create_bitmap("bitmap_height".to_string(), 100, 100);
...
fn test_bitmap_width_of_bitmap_named_integration() {
    let test_bitmap = create_bitmap("bitmap_width".to_string(), 100, 100);
...
fn test_load_bitmap_integration() {
    ...
    assert_eq!(color_black(), get_pixel_from_bitmap(loaded_bitmap, 50.0, 50.0));
    assert_eq!(color_white(), get_pixel_from_bitmap(loaded_bitmap, 75.0, 75.0));
...
fn test_text_height_font_named_integration() {
    let test_window = open_window("Test Window".to_string(), 800, 600);
    let height = text_height_font_named("Test Text".to_string(), "hara".to_string(), 24);
    assert!(height > 0);
    assert!(height >= 24);
    close_window(test_window);
}
