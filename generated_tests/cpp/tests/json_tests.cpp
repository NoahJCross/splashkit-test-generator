...
TEST_CASE("json_from_file_integration") {
    ...
    REQUIRE("expected_value" == test_value);
    free_json(test_json);
}
...
TEST_CASE("json_read_array_of_double_integration") {
    auto test_json = create_json();
    json_set_array(test_json, "numbers", std::vector<double> { 1.1, 2.2, 3.3 };);
...
TEST_CASE("json_read_array_of_bool_integration") {
    auto test_json = create_json();
    json_set_array(test_json, "test_key", std::vector<bool> { true, false, true };);
    auto out_result = std::vector<bool> {  };
    json_read_array(test_json, "test_key", out_result);
    REQUIRE(size(out_result) == 3);
...
TEST_CASE("json_set_array_of_string_integration") {
    auto test_json = create_json();
    json_set_array(test_json, "test_key", std::vector<string> { "value1", "value2", "value3" };);
    auto out_result = std::vector<string> {  };
    json_read_array(test_json, "test_key", out_result);
...
TEST_CASE("json_set_number_float_integration") {
    auto test_json = create_json();
    json_set_number(test_json, "test_key", 3.14);
    REQUIRE(3.14 == json_read_number(test_json, "test_key"));
    free_json(test_json);
}
