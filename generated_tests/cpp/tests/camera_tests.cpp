...
public:
    ...
    TEST_CASE("center_camera_on_vector_integration") {
        auto test_window = open_window(string("Test Window"), 800, 600);
        auto test_sprite = create_sprite(string("test_sprite"));
    ...
    TEST_CASE("to_screen_rectangle_integration") {
        ...
        REQUIRE(to_screen_x(100.0)f == rectangle_left(screen_rectangle));
        REQUIRE(to_screen_y(100.0)f == rectangle_top(screen_rectangle));
        close_window(test_window);
    }
